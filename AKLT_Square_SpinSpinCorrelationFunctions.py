            ...
            if (gammaln(N_faces+1)-gammaln(n+1)-gammaln(N_faces-n+1)) > np.log(samples):
                ...
                for end_node in range(2, int(N_x / 2) + 1 + 1):
                    ...
                    Bs_components_dict[end_node][n - n_low] = (-1)**parity*\
                        np.mean(np.abs(Bs_component_contributions_dict[end_node]))*\
                        np.exp(gammaln(N_faces + 1) - gammaln(n + 1) - gammaln(N_faces - n + 1))
            else:
...
if __name__ == '__main__':
    ...
    deg2_weights = np.array([0.1, 0.2])             # Weights of degree 2 vertex
    CR_weight = 1/15                                # Weight of crossing
    CP_weight = CR_weight                           # Weight of a corner pass
    loop_weight = .2                                # Weight of a closed loop
    ...
    CorrelationFunctions = np.zeros([int(N_x/2), len(deg2_weights)])
    ...
    for deg2_index in range(len(deg2_weights)):
        ...
        for i in range(int(N_x/2)):
            CorrelationFunctions[i, deg2_index] = 3*B[i]/A

        print('\n Correlation Functions:')
        for func in CorrelationFunctions[:, deg2_index]:
            print(func)

        # Fits correlation functions to exponentials, extracts correlation length from log fit and exponential fit
        LogCorrelationFunctions = [np.log(np.abs(CorrelationFunctions[i, deg2_index])) for i in range(int(N_x/2))]
        m, b, r_value, p_value, std_err = \
            stats.linregress(range(1, len(LogCorrelationFunctions) + 1), LogCorrelationFunctions)
        correlation_lengths_log[deg2_index] = -1/m
        popt, pcov = curve_fit(exp_correlation_function, list(range(1, int(N_x/2) + 1)),
                               np.abs(CorrelationFunctions[0:int(N_x/2), deg2_index]), p0=[np.exp(b), -1/m])
